...
class RegisterClass:
    def __init__(self, root):
        self.root = root
        self.root.title("Register Page | sikandarsingh2210@gmail.com | +91-7057525359")
        ...
        Label(self.root, text = "Registration", font = ("goudy old style", 32, "bold"), fg = "darkred", bg = "white", bd = 0).place(x = 0, y = 20, relwidth = 1, height = 70)
        ...
        Entry(self.root, textvariable = self.username, bd = 2, relief = SUNKEN, font = ("goudy old style", 20), bg = "white").place(x = 200, y = 140, width = 230)

        Label(self.root, text = "Email", font = ("goudy old style", 20, "bold"), fg = "darkblue", bg = "white").place(x = 470, y = 140)
        Entry(self.root, textvariable = self.email, bd = 2, relief = SUNKEN, font = ("goudy old style", 20), bg = "white").place(x = 560, y = 140, width = 360)

        Label(self.root, text = "Password", font = ("goudy old style", 20, "bold"), fg = "darkblue", bg = "white").place(x = 65, y = 220)
        ...
        Label(self.root, text = "( Note : Select the graphical password from the above and please remember the pictures you select &\nPlease enter the valid email for the verification. )", anchor = "w", fg = "black", bg = "white", font = ("", 10)).place(x = 200, y = 550)
        ...
        self.register_btn.place(x = 200, y = 600, width = 300, height = 50)
    ...
    def register_cmd(self):
        try:
            if self.username.get() == "" or self.email.get() == "" or self.expression == "":
                msg.showerror("Fields Error","All fields are required !!", parent = self.root)
            else:
                cur.execute("select username from gaps_data where username = ?",(self.username.get(),))
                result = cur.fetchall()
                
                if len(result) > 0:
                    msg.showerror("Account Error",f"This {self.username.get()} is already exists.\nTry again !!", parent = self.root)
                else:
                    query = "insert into gaps_data values (?,?,?,?)"
                    value = (rd.randint(11, 99), self.username.get(), self.email.get(), self.expression)

                    cur.execute(query, value)
                    conn.commit()

                    msg.showinfo("Register Success","User registered successfully...", parent = self.root)
                    self.clear_cmd()
